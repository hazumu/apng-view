...
public class ApngDrawable extends Drawable implements Animatable, Runnable {
    ...
    public ApngDrawable(Context context, Bitmap bitmap, Uri uri) {
        ...
        displayImageOptions = new DisplayImageOptions.Builder().cacheInMemory(
            false) // prevent GC_ALLOC, which cause lacking while playing
    ...
    private void drawAnimateBitmap(Canvas canvas, int frameIndex) {
        ...
        if (bitmap == null) {
            try {
                ...
                if (enableVerboseLog) FLog.v("Error: " + e.getMessage());
                return;
            }
    ...
    private Bitmap createAnimateBitmap(int frameIndex) {
        ...
        String path =
            new File(workingPath, ApngExtractFrames.getFileName(baseFile, frameIndex)).getPath();
        Bitmap frameBitmap =
            imageLoader.loadImageSync(Uri.fromFile(new File(path)).toString(), displayImageOptions);
    ...
    private Bitmap handleDisposeOperation(int frameIndex, File baseFile,
        ...
        switch (disposeOp) {
            ...
            case PngChunkFCTL.APNG_DISPOSE_OP_BACKGROUND:
                ...
                tempPath = new File(workingPath,
                    ApngExtractFrames.getFileName(baseFile, frameIndex - 1)).getPath();
                frameBitmap = imageLoader.loadImageSync(Uri.fromFile(new File(tempPath)).toString(),
                    displayImageOptions);
                ...
                tempCanvas.clipRect(offsetX, offsetY, offsetX + frameBitmap.getWidth(),
                    offsetY + frameBitmap.getHeight());
            ...
            case PngChunkFCTL.APNG_DISPOSE_OP_PREVIOUS:
                if (frameIndex > 1) {
                    ...
                    for (int i = frameIndex - 2; i >= 0; i--) {
                        ...
                        tempPath = new File(workingPath, ApngExtractFrames.getFileName(baseFile, i))
                            .getPath();
                        frameBitmap =
                            imageLoader.loadImageSync(Uri.fromFile(new File(tempPath)).toString(),
                                displayImageOptions);
                        ...
                        if (tempDisposeOp != PngChunkFCTL.APNG_DISPOSE_OP_PREVIOUS) {
                            ...
                            if (tempDisposeOp == PngChunkFCTL.APNG_DISPOSE_OP_NONE) {
                                ...
                                if (bitmap == null) {
                                    FLog.w(
                                        "Can't retrieve previous APNG_DISPOSE_OP_NONE frame: please try to increase memory cache size!");
                                }
                            } else if (tempDisposeOp == PngChunkFCTL.APNG_DISPOSE_OP_BACKGROUND) {
                                if (enableVerboseLog) FLog.v("Create a new bitmap");
                                tempBitmap = Bitmap.createBitmap(baseWidth, baseHeight,
                                    Bitmap.Config.ARGB_8888);
                                tempCanvas = new Canvas(tempBitmap);
                                tempCanvas.drawBitmap(getCacheBitmap(i), 0, 0, null);

                                tempCanvas.clipRect(tempOffsetX, tempOffsetY,
                                    tempOffsetX + frameBitmap.getWidth(),
                                    tempOffsetY + frameBitmap.getHeight());
                                tempCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
                                tempCanvas.clipRect(0, 0, baseWidth, baseHeight);
    ...
    private String getImagePathFromUri() {
        ...
        try {
            ...
            if (!file.exists()) {
                if (enableVerboseLog) {
                    FLog.v("Copy file from %s to %s", sourceUri.getPath(), file.getPath());
                }
     ...
     */
    private Bitmap handleBlendingOperation(int offsetX, int offsetY, byte blendOp,
        ...
        if (baseBitmap != null) {
            ...
            if (blendOp == PngChunkFCTL.APNG_BLEND_OP_SOURCE) {
                canvas.clipRect(offsetX, offsetY, offsetX + frameBitmap.getWidth(),
                    offsetY + frameBitmap.getHeight());
